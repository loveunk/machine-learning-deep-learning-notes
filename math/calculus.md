...
虽然雅可比矩阵可以显示梯度，但它无法提供足够的信息来判断一个点是局部极大还是最小值。这正是黑森矩阵的作用所在。黑森矩阵是函数的二阶导数，提供了函数曲率的更详细分析。
这有助于识别临界点的性质（无论是极大点、极小值还是鞍点）。
...
* _|H| > 0_：
  ...
  *  如果 _H<sub>[0,0]</sub> < 0_，则 _(x<sub>0</sub>, y<sub>0</sub>)_ 为局部最大值
    <img width="1265" height="249" alt="image" src="https://github.com/user-attachments/assets/b6bed75c-d4c4-47bd-8a7b-51c3ae665993" />

* _|H| = 0_：
  * 则 _(x<sub>0</sub>, y<sub>0</sub>)_ 为鞍点（即非局部极值）
  * 如果黑森矩阵的行列式为零，说明函数的曲率无法提供足够信息来分类临界点。换句话说，它是一个鞍点，函数的表面没有沿一致方向“弯曲”。
* _|H| < 0_：
  * 二阶导数无法判断该临界点的性质，得从更高阶的导数以泰勒公式来判断
  * 如果黑森行列式为负，则需要更高阶导数来确定临界点的性质。
   解释 ：如果黑森行列式为负，说明函数表现出混合曲率（即既有凸区也有凹面），因此需要更高阶导数或更详细的分析（如使用克雷因准则 ）来分类临界点。
